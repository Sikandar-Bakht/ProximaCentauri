...
class SprintFiveProjStack(cdk.Stack):
    ...
    def __init__(self, scope: cdk.Construct, construct_id: str, **kwargs) -> None:
        ...
        print("Sprint 4 Project stack instantiated :)")
        ...
        WH_Lambda = self.create_lambda("SikandarS4WebHealthLambda", "./resources/", "WH_Lambda.lambda_handler", lambda_role,
                                        cdk.Duration.seconds(20))
                            
        lambda_schedule = events_.Schedule.rate(cdk.Duration.minutes(1))
        lambda_target = targets_.LambdaFunction(handler=WH_Lambda)
        rule = events_.Rule(self, "S4webHealth_Invocation", description="Periodic Lambda", enabled=True, schedule=lambda_schedule, 
        ...
        db_table = self.create_db_table(id = "SprintFourAlarmTable", part_key=db.Attribute(name="Timestamp", type=db.AttributeType.STRING))
        db_lambda_role = self.create_db_lambda_role()
        DB_Lambda = self.create_lambda("SikandarS4DBLambda", "./resources/", "DB_Lambda.lambda_handler", role=db_lambda_role)
        ...
        monitor_api = apigateway.LambdaRestApi(self, "SikandarS4_API",
                                handler=API_Lambda
                                )
        ...
        cluster = ecs.Cluster(self, "Sikandar_cluster", vpc=vpc)
